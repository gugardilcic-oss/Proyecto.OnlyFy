...
import com.onlyfy.payment_service.service.PaymentService;
...
public class PaymentController {
    ...
    @PostMapping
    public ResponseEntity<?> crearPago(@RequestBody Payment payment) {
        if (!paymentService.datosValidos(payment)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Datos del pago inválidos");
        }
        Payment nuevo = paymentService.crearPago(payment);
        return ResponseEntity.status(HttpStatus.CREATED).body(nuevo);
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> actualizarPago(@PathVariable int id, @RequestBody Payment payment) {
        if (!paymentService.datosValidos(payment)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Datos del pago inválidos");
        }
